...
def longestMaxSubSeq(seq: list[Union[int, float]]) -> tuple[Union[float, int], list[int]]:
    # variables
    _current_sum = 0
    max_sum = -float('inf')
    _indexes = []
    ...
    """
       ...
       :type _current_sum: every element of list we go through will
            be added to this variable
       :type _indexes: will hold the first index of the the subsequence
       :type max_sum: will hold the max sum of the longest subsequence
       :type _last_index: will hold the last index of subsequence
    """
